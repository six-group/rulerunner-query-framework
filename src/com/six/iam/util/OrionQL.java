...
public class OrionQL
{
	 ...
	 * evaluated on them. By implementing the {@link CerberusLogic.IAccessor}
		...
		Object access(Object source, Proxy<?> context)
		{
			...
			while (input.hasNext()) {
				...
				if (element != null)
				{
					if (isHtml == null)
					{
						isHtml = (element instanceof HtmlContainer);
					}
					result.append(
							(isHtml && !(element instanceof HtmlContainer)) ?
			...
			return (isHtml == Boolean.TRUE) ? new HtmlContainer(result.toString(), null) : result.toString();
		}
	}
	...
	private static class ConstructInstruction extends ProcessingInstruction
	{
		...
		Object access(Object source, Proxy<?> context)
		{
			...
			for (var accessor: accessors)
			{
				...
				{
					var input = Accessor.asIterable(value).iterator();
