...
public class OrionQLTest
{
	...
	private final Identity manager = createIdentity("usr000", "Hans", "Muster", "ORG", null, null);
	private final Bundle bundle_Role029 = createBundle("Role029", "Archive - Admin", "Archive", null);
	private final Bundle bundle_Role044 = createBundle("Role044", "Finance Information - Admin", "FinInfo", null);
	private final Bundle bundle_Role060 = createBundle("Role060", "Clearing System Admin", "Clearing", manager);
	private final Identity identity = createIdentity("usr001", "John", "Doe", "ORG-DOE", manager, List.of(bundle_Role029, bundle_Role044, bundle_Role060));
	private final Identity subordinate = createIdentity("usr002", "Calpurnius", "Blandus", "ORG-DOE", identity, List.of(bundle_Role029, bundle_Role060));
	private final Bundle bundle = createBundle("Role111", "ControlM Prod - Server Access Applikationsadmin Control-M (ux)", "ControlM", identity);
	...
	public void test_convenience_methods()
	{
		...
		var input = identity;
		// Act & Assert
		assertThat("Incorrect result", OrionQL.evaluate(input, "assignedRoles:map(name)", null), is(List.of("Role029", "Role044", "Role060")));
		assertThat("Incorrect result", OrionQL.format(input, "${firstname} ${lastname} (${name})", null), is("John Doe (usr001)"));
	...
	public void test_expression_macros()
	{
		...
		// Act & Assert
		assertThat("Incorrect result", proxy.access("$managerRolesList$:join"), is("Archive - Admin (Role029)\nFinance Information - Admin (Role044)\nClearing System Admin (Role060)"));
	...
	public void test_template_macros()
	{
		...
		assertThat("Incorrect result", accessor.access(proxy), is("John Doe, ORG-DOE: 3 roles"));
	}
	...
	public void test_expression_macros_multicolumn()
	{
		...
		// Act & Assert
		assertThat("Incorrect result", proxy.access("assignedRoles:table($columns$,nameLength=$displayname$:size)").toString(), is("<table><thead><tr><th>id</th><th>name</th><th>owner</th><th>nameLength</th></tr></thead><tbody><tr><td>Role029</td><td>Archive - Admin</td><td></td><td>15</td></tr><tr><td>Role044</td><td>Finance Information - Admin</td><td></td><td>27</td></tr><tr><td>Role060</td><td>Clearing System Admin</td><td>Hans Muster (usr000)</td><td>21</td></tr></tbody></table>"));
	}
	...
	public void test_parameterized_attribute_access()
	{
		...
		// Act & Assert
		assertThat("Incorrect attribute value", proxy.access("owner.attribute(:format(orgUnit))"), is("ORG-DOE"));
		assertThat("Incorrect attribute value", proxy.access("owner.attribute(attributeName)"), is("ORG-DOE"));
		assertThat("Incorrect attribute value", proxy.access("description(:format(en_US))"), is("ControlM Prod - Server Access Applikationsadmin Control-M (ux) (Role111)"));
	}
	...
	public void test_formatting()
	{
		...
		assertThat("Incorrect formatting result", proxy.access(":format(${firstname} ${lastname} (${name}))"), is("John Doe (usr001)"));
		assertThat("Incorrect formatting result", proxy.access(":format(${firstname} ${lastname} \\(${name})"), is("John Doe (usr001"));
		assertThat("Incorrect formatting result", proxy.access(":format(${firstname} ${lastname} \\(${name}\\))"), is("John Doe (usr001)"));
		assertThat("Incorrect formatting result", proxy.access(":format(${firstname} ${lastname} ((${name}), \"1) \\\"foo\\\", 2)\\\"bar\\\"\"))"), is("John Doe ((usr001), \"1) \\\"foo\\\", 2)\\\"bar\\\"\")"));
		...
		assertThat("Incorrect formatting result", proxy.access(":format(${%20s,name,--unknown--})"), is("              usr001"));
		...
		assertThat("Incorrect list formatting", proxy.access("assignedRoles:map(name):format($\"\")"), is("{\"Role029\", \"Role044\", \"Role060\"}"));
		assertThat("Incorrect formatting result", proxy.access(":format(${%[%s] ,logonid,})"), is(""));
		assertThat("Incorrect formatting result", proxy.access(":format(${%[%s] ,name,})"), is("[usr001] "));
		assertThat("Incorrect date formatting", proxy.access(":format(${%'['yyyy-MM-dd HH:mm:ss']: ',startDate})"), is("[2022-04-07 16:49:13]: "));
		assertThat("Incorrect date formatting", proxy.access(":format(${%[%tF %1$tT]: ,startDate})"), is("[2022-04-07 16:49:13]: "));
	}
	...
	public void test_html()
	{
		...
		proxy.implant("motd", "Life is -> beautiful");
		// Act & Assert
		assertThat("Incorrect HTML", proxy.access(":link(/identity/${name},${displayName})").toString(), is("<a href=\"/identity/usr001\" target=\"_blank\">John Doe</a>"));
		...
		assertThat("Incorrect HTML", proxy.access(":link(/identity/${name},${displayName},<h1>${motd}</h1>)").toString().replaceAll("\"[0-9]+,[-0-9]+\"", "\"nnnn,nnnn\""), is("<span class=\"pinnable-tooltip-container\"><a href=\"/identity/usr001\" target=\"_blank\">John Doe</a><input type=\"checkbox\" id=\"nnnn,nnnn\"><label for=\"nnnn,nnnn\"></label><div class=\"pinnable-tooltip-content\"><h1>Life is -&gt; beautiful</h1></div></span>"));
		...
		assertThat("Incorrect HTML", proxy.access(":html(<div>${:link(/identity/${name},${displayName})}: ${motd}</div>)").toString(), is("<div><a href=\"/identity/usr001\" target=\"_blank\">John Doe</a>: Life is -&gt; beautiful</div>"));
		...
		assertThat("Incorrect HTML", proxy.access("assignedRoles:map(:link(/bundle/${name},${displayName})):join").toString(), is("<a href=\"/bundle/Role029\" target=\"_blank\">Archive - Admin</a>\n<a href=\"/bundle/Role044\" target=\"_blank\">Finance Information - Admin</a>\n<a href=\"/bundle/Role060\" target=\"_blank\">Clearing System Admin</a>"));
	}
	...
	{
		// Arrange
		var proxy = OrionQL.wrap(identity, null, null, Literals.asMap("baseurl", "https://identitiq.localdomain.com/identityiq/rulerunner/"));
		// Act & Assert
		assertThat("Incorrect HTML", proxy.access(":link(identity/${name},${displayName})").toString(), is("<a href=\"https://identitiq.localdomain.com/identityiq/rulerunner/identity/usr001\" target=\"_blank\">John Doe</a>"));
		assertThat("Incorrect HTML", proxy.access(":link(/identity/${name},${displayName})").toString(), is("<a href=\"https://identitiq.localdomain.com/identity/usr001\" target=\"_blank\">John Doe</a>"));
		assertThat("Incorrect HTML", proxy.access(":link(https://identitiq.localdomain.com/identity/${name},${displayName})").toString(), is("<a href=\"https://identitiq.localdomain.com/identity/usr001\" target=\"_blank\">John Doe</a>"));
	}
	...
	public void test_html_table_grid()
	{
		...
		var proxy = OrionQL.wrap(identity, null);
		// Act & Assert
		assertThat("Incorrect HTML", proxy.access("assignedRoles:table(identity=^.name,name,displayName)").toString(), is("<table><thead><tr><th>identity</th><th>name</th><th>displayName</th></tr></thead><tbody><tr><td>usr001</td><td>Role029</td><td>Archive - Admin</td></tr><tr><td>usr001</td><td>Role044</td><td>Finance Information - Admin</td></tr><tr><td>usr001</td><td>Role060</td><td>Clearing System Admin</td></tr></tbody></table>"));
		...
		assertThat("Incorrect HTML", proxy.access(":htable(name,displayName,assignedRoles:size)").toString(), is("<table><tbody><tr><th>name</th><td>usr001</td></tr><tr><th>displayName</th><td>John Doe</td></tr><tr><th>assignedRoles:size</th><td>3</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:index(>name#^.name,\"*\"):grid").toString(), is("<table><thead><tr><th></th><th>usr001</th></tr></thead><tbody><tr><th>Role029</th><td>*</td></tr><tr><th>Role044</th><td>*</td></tr><tr><th>Role060</th><td>*</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:index(>name#^.name,1):grid").toString(), is("<table><thead><tr><th></th><th>usr001</th></tr></thead><tbody><tr><th>Role029</th><td class=\"numeric\">1</td></tr><tr><th>Role044</th><td class=\"numeric\">1</td></tr><tr><th>Role060</th><td class=\"numeric\">1</td></tr><tr><th>TOTAL</th><td class=\"numeric\">3</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:index(>name#^.name,1):grid()").toString(), is("<table><thead><tr><th></th><th>usr001</th></tr></thead><tbody><tr><th>Role029</th><td>1</td></tr><tr><th>Role044</th><td>1</td></tr><tr><th>Role060</th><td>1</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:group(>name#^.name,\"*\"):grid").toString(), is("<table><thead><tr><th></th><th>usr001</th></tr></thead><tbody><tr><th>Role029</th><td>*</td></tr><tr><th>Role044</th><td>*</td></tr><tr><th>Role060</th><td>*</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:group(>name#^.name,\"*\"):grid").getClass().getSimpleName(), is("HtmlContainer"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:group(>name:size#^.name,\"*\"):grid").toString(), is("<table><thead><tr><th></th><th>usr001</th></tr></thead><tbody><tr><th>7</th><td>*\n*\n*</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:group(>name:size#^.name,\"*\"):grid(:join( ))").toString(), is("<table><thead><tr><th></th><th>usr001</th></tr></thead><tbody><tr><th>7</th><td>* * *</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:group(>name#\"\",\"*\"):grid").toString(), is("<table><tbody><tr><th>Role029</th><td>*</td></tr><tr><th>Role044</th><td>*</td></tr><tr><th>Role060</th><td>*</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:count(>name#\"assignments\"):grid()").toString(), is("<table><thead><tr><th></th><th>assignments</th></tr></thead><tbody><tr><th>Role029</th><td>1</td></tr><tr><th>Role044</th><td>1</td></tr><tr><th>Role060</th><td>1</td></tr></tbody></table>"));
		...
		assertThat("Incorrect HTML", proxy.access("assignedRoles:count(>name:size#\"assignments\"#name):grid").toString(), is("<table><thead><tr><th></th><th>assignments</th></tr></thead><tbody><tr><th>7</th><td>Role029 = 1\nRole044 = 1\nRole060 = 1</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:count(>name:size#\"assignments\"#name):grid(:pprint)").toString(), is("<table><thead><tr><th></th><th>assignments</th></tr></thead><tbody><tr><th>7</th><td>Role029 = 1\nRole044 = 1\nRole060 = 1</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:count(>name:size#\"assignments\"#name):grid(,1)").toString(), is("\n<h2 title=\"7\">7</h2>\n<table><thead><tr><th></th><th>Role029</th><th>Role044</th><th>Role060</th></tr></thead><tbody><tr><th>assignments</th><td>1</td><td>1</td><td>1</td></tr></tbody></table>\n"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:count(>name:size#\"assignments\"#name):grid(,1,totals)").toString(), is("\n<h2 title=\"7\">7</h2>\n<table><thead><tr><th></th><th>Role029</th><th>Role044</th><th>Role060</th><th>TOTAL</th></tr></thead><tbody><tr><th>assignments</th><td class=\"numeric\">1</td><td class=\"numeric\">1</td><td class=\"numeric\">1</td><td class=\"numeric\">3</td></tr><tr><th>TOTAL</th><td class=\"numeric\">1</td><td class=\"numeric\">1</td><td class=\"numeric\">1</td><td class=\"numeric\">3</td></tr></tbody></table>\n"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:index(>name:size#name,:link(#,${1})):grid").toString(), is("<table><thead><tr><th></th><th>Role029</th><th>Role044</th><th>Role060</th><th>TOTAL</th></tr></thead><tbody><tr><th>7</th><td class=\"numeric\"><span>1</span></td><td class=\"numeric\"><span>1</span></td><td class=\"numeric\"><span>1</span></td><td class=\"numeric\">3</td></tr><tr><th>TOTAL</th><td class=\"numeric\">1</td><td class=\"numeric\">1</td><td class=\"numeric\">1</td><td class=\"numeric\">3</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:index(>name:size#name,:link(#,${1:barchart})):grid").toString(), is("<table><thead><tr><th></th><th>Role029</th><th>Role044</th><th>Role060</th><th>TOTAL</th></tr></thead><tbody><tr><th>7</th><td class=\"numeric\"><span><span class=\"chart\"><span title=\"1\" style=\"background-color: black; cursor: pointer;\">&hairsp;</span></span></span></td><td class=\"numeric\"><span><span class=\"chart\"><span title=\"1\" style=\"background-color: black; cursor: pointer;\">&hairsp;</span></span></span></td><td class=\"numeric\"><span><span class=\"chart\"><span title=\"1\" style=\"background-color: black; cursor: pointer;\">&hairsp;</span></span></span></td><td class=\"numeric\">3</td></tr><tr><th>TOTAL</th><td class=\"numeric\">1</td><td class=\"numeric\">1</td><td class=\"numeric\">1</td><td class=\"numeric\">3</td></tr></tbody></table>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:index(>name:size#name,:link(#,${name:extract([0-9]):count(>):barchart(*=.)})):grid").toString(), is("<table><thead><tr><th></th><th>Role029</th><th>Role044</th><th>Role060</th><th>TOTAL</th></tr></thead><tbody><tr><th>7</th><td class=\"numeric\"><span><span class=\"chart\"><span title=\"0: 1\" style=\"background-color: #cfcd20; cursor: pointer;\">&hairsp;</span><span title=\"2: 1\" style=\"background-color: #c81e72; cursor: pointer;\">&hairsp;</span><span title=\"9: 1\" style=\"background-color: #45c48c; cursor: pointer;\">&hairsp;</span></span></span></td><td class=\"numeric\"><span><span class=\"chart\"><span title=\"0: 1\" style=\"background-color: #cfcd20; cursor: pointer;\">&hairsp;</span><span title=\"4: 2\" style=\"background-color: #a87ff6; cursor: pointer;\">&hairsp;&hairsp;</span></span></span></td><td class=\"numeric\"><span><span class=\"chart\"><span title=\"0: 2\" style=\"background-color: #cfcd20; cursor: pointer;\">&hairsp;&hairsp;</span><span title=\"6: 1\" style=\"background-color: #167909; cursor: pointer;\">&hairsp;</span></span></span></td><td class=\"numeric\">9</td></tr><tr><th>TOTAL</th><td class=\"numeric\">3</td><td class=\"numeric\">3</td><td class=\"numeric\">3</td><td class=\"numeric\">9</td></tr></tbody></table>"));
	}
	...
	public void test_chart()
	{
		...
		assertThat("Incorrect HTML", proxy.access("assignedRoles:barchart(*=:count(>displayName:match(-? Admin)))").toString(), is("<span class=\"chart\"><span title=\" Admin: 1\" style=\"background-color: #f42e7c; cursor: pointer;\">&hairsp;</span><span title=\"- Admin: 2\" style=\"background-color: #0e2ea7; cursor: pointer;\">&hairsp;&hairsp;</span></span>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:barchart(*=:count(>displayName:size/10))").toString(), is("<span class=\"chart\"><span title=\"1: 1\" style=\"background-color: #c4ca42; cursor: pointer;\">&hairsp;</span><span title=\"2: 2\" style=\"background-color: #c81e72; cursor: pointer;\">&hairsp;&hairsp;</span></span>"));
		...
		assertThat("Incorrect HTML", proxy.access("assignedRoles:linechart(*=:count(>displayName:match(-? Admin)))").toString(), is("<span class=\"chart\"><span style=\"background-color: black;\">&thinsp;</span><span title=\" Admin: 1\" style=\"background-color: #f42e7c; cursor: pointer;\">&thinsp;</span><span title=\"- Admin: 2\" style=\"background-color: #0e2ea7; cursor: pointer;\">&thinsp;</span></span>"));
		assertThat("Incorrect HTML", proxy.access("assignedRoles:linechart(*=:count(>displayName:size/10))").toString(), is("<span class=\"chart\"><span style=\"background-color: black;\">&thinsp;</span><span title=\"1: 1\" style=\"background-color: #c4ca42; cursor: pointer;\">&thinsp;</span><span title=\"2: 2\" style=\"background-color: #c81e72; cursor: pointer;\">&thinsp;</span></span>"));
	}
	...
	public void test_tostring()
	{
		...
		assertThat("Incorrect formatting result", proxy.toString(), is("John Doe (usr001)"));
	}
	...
	public void test_map_join()
	{
		...
		// Act & Assert
		assertThat("Incorrect mapping result", proxy.access("assignedRoles:map(name)"), is(List.of("Role029", "Role044", "Role060")));
		assertThat("Incorrect mapping result", proxy.access("assignedRoles:map(:format(<${name}>))"), is(List.of("<Role029>", "<Role044>", "<Role060>")));
		assertThat("Incorrect mapping result", proxy.access("assignedRoles:map(name:format(<${}>))"), is(List.of("<Role029>", "<Role044>", "<Role060>")));
		assertThat("Incorrect mapping result", proxy.access("assignedRoles:map(name):join").toString(), is("Role029\nRole044\nRole060"));
		assertThat("Incorrect mapping result", proxy.access("assignedRoles:map(name):join(,)").toString(), is("Role029,Role044,Role060"));
		assertThat("Incorrect mapping result", proxy.access("assignedRoles:map(:format(${^.name} -> ${name}))"), is(List.of("usr001 -> Role029" ,"usr001 -> Role044", "usr001 -> Role060")));
		assertThat("Incorrect mapping result", proxy.access("qotd:map(:format(${^.name}: ${key} -> ${value}))"), is(List.of("usr001: Monday -> Life is beautiful" ,"usr001: Tuesday -> Don't worry, be happy")));
	...
	public void test_proxy_chain()
	{
		...
		// Act & Assert
		assertThat("Incorrect formatting", childProxy.toString(), is("John Smith (usr001)"));
	...
	public void test_lookup()
	{
		// Arrange
		var proxy = OrionQL.wrap(identity, null)
				.setLookupTable("nickname", Literals.asMap("usr001", "Johnny"))
	...
	public void test_db_objects_lookup() throws GeneralException
	{
		...
		assertThat("Incorrect query result", proxy.access(":Identity((name==$\"name\" || name.in({\"usr001\"})))"), is(identity));
		assertThat("Incorrect query result", proxy.access(":Identity(name.in({\"usr001\", \"1) foo, 2) \\\"bar\\\"\"}))"), is(identity));
	...
	public void test_db_lookup_options() throws GeneralException
	{
		// Arrange
		var context = mock(SailPointContext.class);
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")), List.of("name"))).thenReturn(List.of(new Object[]{"usrabc"}, new Object[]{"usr123"}, new Object[]{"usrxyz"}).iterator());
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")).setResultLimit(2), List.of("name"))).thenReturn(List.of(new Object[]{"usrabc"}, new Object[]{"usr123"}).iterator());
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")).setResultLimit(1), List.of("name"))).thenReturn(Collections.singletonList(new Object[]{"usrabc"}).iterator());
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")).addOrdering("name", true), List.of("name"))).thenReturn(List.of(new Object[]{"usr123"}, new Object[]{"usrabc"}, new Object[]{"usrxyz"}).iterator());
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")).addOrdering("name", true).setResultLimit(2), List.of("name"))).thenReturn(List.of(new Object[]{"usr123"}, new Object[]{"usrabc"}).iterator());
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")).addOrdering("name", true).setResultLimit(1), List.of("name"))).thenReturn(Collections.singletonList(new Object[]{"usr123"}).iterator());
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")).addOrdering("name", false), List.of("name"))).thenReturn(List.of(new Object[]{"usrxyz"}, new Object[]{"usrabc"}, new Object[]{"usr123"}).iterator());
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")).setResultLimit(1), List.of("name", "lastname"))).thenReturn(Collections.singletonList(new Object[]{"usrabc", "Doe"}).iterator());
		var proxy = OrionQL.wrap(identity, context);
		// Act & Assert
		assertThat("Incorrect query result", proxy.access(":Identity([{name}department==\"DEPT\"])"), is(List.of("usrabc", "usr123", "usrxyz")));
		assertThat("Incorrect query result", proxy.access(":Identity([{name}department==\"DEPT\",2])"), is(List.of("usrabc", "usr123")));
		assertThat("Incorrect query result", proxy.access(":Identity([{name}department==\"DEPT\",1])"), is("usrabc"));
		assertThat("Incorrect query result", proxy.access(":Identity([{name}department==\"DEPT\",name])"), is(List.of("usr123", "usrabc", "usrxyz")));
		assertThat("Incorrect query result", proxy.access(":Identity([{name}department==\"DEPT\",name,2])"), is(List.of("usr123", "usrabc")));
		assertThat("Incorrect query result", proxy.access(":Identity([{name}department==\"DEPT\",name,1])"), is("usr123"));
		assertThat("Incorrect query result", proxy.access(":Identity([{name}department==\"DEPT\",name desc])"), is(List.of("usrxyz", "usrabc", "usr123")));
		assertThat("Incorrect query result", proxy.access(":Identity([{name,lastname}department==\"DEPT\",1])"), equalToObject(Literals.asMap("name", "usrabc").add("lastname", "Doe")));
	}
	...
	public void test_db_lookup_stream_attributes() throws GeneralException
	{
		// Arrange
		var context = mock(SailPointContext.class);
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")), List.of("name"))).thenAnswer(invocationOnMock -> List.of(new Object[]{"usrabc"}, new Object[]{"usr123"}, new Object[]{"usrxyz"}).iterator());
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")).setResultLimit(1), List.of("name"))).thenAnswer(invocationOnMock -> Collections.singletonList(new Object[]{"usrabc"}).iterator());
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")), List.of("name", "id"))).thenReturn(List.of(new Object[]{"usrabc", "1000"}, new Object[]{"usr123", "1001"}, new Object[]{"usrxyz", "1002"}).iterator());
		when(context.search(Identity.class, new QueryOptions().addFilter(Filter.eq("department", "DEPT")).setResultLimit(1), List.of("name", "id"))).thenReturn(Collections.singletonList(new Object[]{"usrabc", "1000"}).iterator());
		var proxy = OrionQL.wrap(identity, context);
		// Act & Assert
		assertThat("Incorrect query result", Util.iteratorToList(((Iterable<Object>) proxy.access(":Identity(>{name}department==\"DEPT\")")).iterator()), is(List.of("usrabc", "usr123", "usrxyz")));
		assertThat("Incorrect query result", Util.iteratorToList(((Iterable<Object>) proxy.access(":Identity(>{name}department==\"DEPT\")")).iterator()), is(List.of("usrabc", "usr123", "usrxyz")));
		assertThat("Incorrect query result", proxy.access(":Identity(>{name}department==\"DEPT\",1)"), is("usrabc"));
		assertThat("Incorrect query result", proxy.access(":Identity(>{name}department==\"DEPT\",1)"), is("usrabc"));
		assertThat("Incorrect query result", Util.iteratorToList(((Iterable<Object>) proxy.access(":Identity(>{name,id}department==\"DEPT\")")).iterator()), is(List.of(Literals.asMap("name", "usrabc").add("id", "1000"), Literals.asMap("name", "usr123").add("id", "1001"), Literals.asMap("name", "usrxyz").add("id", "1002"))));
		assertThat("Incorrect query result", proxy.access(":Identity(>{name,id}department==\"DEPT\",1)"), equalToObject(Literals.asMap("name", "usrabc").add("id", "1000")));
	}
	...
	public void test_cache()
	{
		...
		assertThat("Incorrect caching", identityProxy.access(":cache(${name}.assignedRoles)"), is(List.of("Role029", "Role044", "Role060")));
		...
		assertThat("Incorrect caching", identityProxy.access(":cache(${name}.assignedRoles,assignedRoles:map(displayName))"), is(List.of(
				"Role029", "Role044", "Role060")));
		identityProxy.clearCache();
		assertThat("Incorrect caching", identityProxy.access(":cache(${name}.assignedRoles,assignedRoles:map(displayName))"), is(List.of(
				"Archive - Admin", "Finance Information - Admin", "Clearing System Admin")));
	}
	...
	public void test_union()
	{
		...
		var proxy = OrionQL.wrap(identity, null);
		// Act & Assert
		assertThat("Incorrect union", proxy.access(":union(name,firstname,nickname,assignedRoles:map(name))"), is(List.of("usr001", "John", "Role029", "Role044", "Role060")));
	}
	...
	public void test_construct()
	{
		...
		var map = Literals.asMap("logonid", "usr001").add("startDate", "2024-04-23");
	...
	public void test_recursive_flatten()
	{
		...
		assertThat("Incorrect walk", proxy.access(":flatten(manager*#assignedRoles):map(:format(${^.firstname}:${name})):join").toString(), is("Calpurnius:Role029\nCalpurnius:Role060\nJohn:Role029\nJohn:Role044\nJohn:Role060"));
		assertThat("Incorrect walk", proxy.access("manager:flatten(owner|assignedRoles*):map(name):join").toString(), is("usr001\nRole029\nRole044\nRole060\nusr000"));
	}
	...
	public void test_flatten_output_processing()
	{
		...
		var proxy = OrionQL.wrap(subordinate, null);
		// Act & Assert
		assertThat("Incorrect sort", proxy.access(":flatten(manager*#assignedRoles):sort(name):map(:format(${^.firstname}:${name})):join").toString(), is("Calpurnius:Role029\nJohn:Role029\nJohn:Role044\nCalpurnius:Role060\nJohn:Role060"));
		assertThat("Incorrect select", proxy.access(":flatten(manager*#assignedRoles):select(eq(\"name\", \"Role029\")):map(:format(${^.firstname}:${name})):join").toString(), is("Calpurnius:Role029\nJohn:Role029"));
		assertThat("Incorrect select", proxy.access(":flatten(manager*#assignedRoles):select(1,eq(\"name\", \"Role029\")):format(${^.firstname}:${name})"), is("Calpurnius:Role029"));
	}
	...
	public void test_simple_conversions()
	{
		...
		assertThat("Incorrect conversion", proxy.access(":toXml"), is("<Identity created=\"1653985801000\" name=\"usr001\">\n  <Attributes>\n    <Map>\n      <entry key=\"firstname\" value=\"John\"/>\n      <entry key=\"lastname\" value=\"Doe\"/>\n      <entry key=\"orgUnit\" value=\"ORG-DOE\"/>\n      <entry key=\"orgUnit.id\" value=\"#ORG-DOE\"/>\n    </Map>\n  </Attributes>\n  <AssignedRoles>\n    <Reference class=\"sailpoint.object.Bundle\" name=\"Role029\"/>\n    <Reference class=\"sailpoint.object.Bundle\" name=\"Role044\"/>\n    <Reference class=\"sailpoint.object.Bundle\" name=\"Role060\"/>\n  </AssignedRoles>\n  <Manager>\n    <Reference class=\"sailpoint.object.Identity\" name=\"usr000\"/>\n  </Manager>\n</Identity>\n"));
		...
		assertThat("Incorrect conversion", proxy.access("assignedRoles:toXml"), is("<List>\n  <Bundle displayName=\"Archive - Admin\" name=\"Role029\">\n    <Attributes>\n      <Map>\n        <entry key=\"application.name\" value=\"Archive\"/>\n        <entry key=\"sysDescriptions\">\n          <value>\n            <Map>\n              <entry key=\"en_US\" value=\"Archive - Admin (Role029)\"/>\n            </Map>\n          </value>\n        </entry>\n      </Map>\n    </Attributes>\n  </Bundle>\n  <Bundle displayName=\"Finance Information - Admin\" name=\"Role044\">\n    <Attributes>\n      <Map>\n        <entry key=\"application.name\" value=\"FinInfo\"/>\n        <entry key=\"sysDescriptions\">\n          <value>\n            <Map>\n              <entry key=\"en_US\" value=\"Finance Information - Admin (Role044)\"/>\n            </Map>\n          </value>\n        </entry>\n      </Map>\n    </Attributes>\n  </Bundle>\n  <Bundle displayName=\"Clearing System Admin\" name=\"Role060\">\n    <Attributes>\n      <Map>\n        <entry key=\"application.name\" value=\"Clearing\"/>\n        <entry key=\"sysDescriptions\">\n          <value>\n            <Map>\n              <entry key=\"en_US\" value=\"Clearing System Admin (Role060)\"/>\n            </Map>\n          </value>\n        </entry>\n      </Map>\n    </Attributes>\n    <Owner>\n      <Reference class=\"sailpoint.object.Identity\" name=\"usr000\"/>\n    </Owner>\n  </Bundle>\n</List>\n"));
	}
	...
	public void test_parse_xml() throws GeneralException
	{
		...
		when(context.getReferencedObject("sailpoint.object.Bundle", null, bundle_Role029.getName())).thenReturn(bundle_Role029);
		when(context.getReferencedObject("sailpoint.object.Bundle", null, bundle_Role044.getName())).thenReturn(bundle_Role044);
		when(context.getReferencedObject("sailpoint.object.Bundle", null, bundle_Role060.getName())).thenReturn(bundle_Role060);
		var proxy = OrionQL.wrap("<Identity created=\"1653985801000\" name=\"usr001\">\n  <Attributes>\n    <Map>\n      <entry key=\"firstname\" value=\"John\"/>\n      <entry key=\"lastname\" value=\"Doe\"/>\n      <entry key=\"orgUnit\" value=\"ORG-DOE\"/>\n      <entry key=\"orgUnit.id\" value=\"#ORG-DOE\"/>\n    </Map>\n  </Attributes>\n  <AssignedRoles>\n    <Reference class=\"sailpoint.object.Bundle\" name=\"Role029\"/>\n    <Reference class=\"sailpoint.object.Bundle\" name=\"Role044\"/>\n    <Reference class=\"sailpoint.object.Bundle\" name=\"Role060\"/>\n  </AssignedRoles>\n  <Manager>\n    <Reference class=\"sailpoint.object.Identity\" name=\"usr000\"/>\n  </Manager>\n</Identity>\n", context);
		// Act & Assert
		assertThat("Incorrect conversion", proxy.access(":parseXml:toXml"), is("<Identity created=\"1653985801000\" name=\"usr001\">\n  <Attributes>\n    <Map>\n      <entry key=\"firstname\" value=\"John\"/>\n      <entry key=\"lastname\" value=\"Doe\"/>\n      <entry key=\"orgUnit\" value=\"ORG-DOE\"/>\n      <entry key=\"orgUnit.id\" value=\"#ORG-DOE\"/>\n    </Map>\n  </Attributes>\n  <AssignedRoles>\n    <Reference class=\"sailpoint.object.Bundle\" name=\"Role029\"/>\n    <Reference class=\"sailpoint.object.Bundle\" name=\"Role044\"/>\n    <Reference class=\"sailpoint.object.Bundle\" name=\"Role060\"/>\n  </AssignedRoles>\n  <Manager>\n    <Reference class=\"sailpoint.object.Identity\" name=\"usr000\"/>\n  </Manager>\n</Identity>\n"));
	}
	...
	public void test_aggregation()
	{
		...
		assertThat("Incorrect result", proxy.access("assignedRoles:sum(displayName:size)"), is(63));
		assertThat("Incorrect result", proxy.access("assignedRoles:avg(displayName:size)"), is(63/3.));
		assertThat("Incorrect result", proxy.access("assignedRoles:max(displayName:size)"), is(27));
		assertThat("Incorrect result", proxy.access("assignedRoles:min(displayName:size)"), is(15));
	}
	...
	public void test_indexing()
	{
		...
		var proxy = OrionQL.wrap(identity, null);
		// Act & Assert
		assertThat("Incorrect aggregation result", proxy.access("assignedRoles:count(>:format(${owner.name}))"), equalToObject(Literals.asMap("null", 2).add("usr000", 1)));
		assertThat("Incorrect aggregation result", proxy.access("assignedRoles:group(>:format(${owner.name}),name)"), equalToObject(Literals.asMap("null", List.of("Role029", "Role044")).add("usr000", List.of("Role060"))));
		assertThat("Incorrect aggregation result", proxy.access("assignedRoles:index(>:format(${owner.name})#name,name)"), equalToObject(Literals.asMap("null", Literals.asMap("Role029", "Role029").add("Role044", "Role044")).add("usr000", Literals.asMap("Role060", "Role060"))));
	}
	...
	public void test_sorting()
	{
		...
		var proxy = OrionQL.wrap(identity, null);
		// Act & Assert
		assertThat("Incorrect sorting", proxy.access("assignedRoles:map(name):sort"), equalToObject(List.of("Role029", "Role044", "Role060")));
		assertThat("Incorrect sorting", proxy.access("assignedRoles:map(name):sort(@ desc)"), equalToObject(List.of("Role060", "Role044", "Role029")));
		assertThat("Incorrect sorting", proxy.access("assignedRoles:map(name):sort(:replace(2,8))"), equalToObject(List.of("Role044", "Role060", "Role029")));
		assertThat("Incorrect sorting", proxy.access("assignedRoles:map(name):sort(:replace(2,8) desc)"), equalToObject(List.of("Role029", "Role060", "Role044")));
		assertThat("Incorrect sorting", proxy.access("assignedRoles:sort(displayName:size):map(name)"), equalToObject(List.of("Role029", "Role060", "Role044")));
		assertThat("Incorrect sorting", proxy.access("assignedRoles:sort(displayName:size desc):map(name)"), equalToObject(List.of("Role044", "Role060", "Role029")));
		assertThat("Incorrect sorting", proxy.access("assignedRoles:sort(owner.name,displayName:size):map(name)"), equalToObject(List.of("Role029", "Role044", "Role060")));
	}
	...
	public void test_select()
	{
		...
		var proxy = OrionQL.wrap(identity, null);
		// Act & Assert
		assertThat("incorrect selection", proxy.access("assignedRoles:select(1)"), is(bundle_Role029));
		assertThat("incorrect selection", proxy.access("assignedRoles:select(2):map(name)"), is(List.of("Role029", "Role044")));
		assertThat("incorrect selection", proxy.access("assignedRoles:select(1,eq(\"name\", \"Role044\")).name"), is("Role044"));
		assertThat("incorrect selection", proxy.access("assignedRoles:select(1,ne(\"name\", \"1) foo\")).name"), is("Role029"));
		assertThat("incorrect selection", proxy.access("assignedRoles:select(1,not(ne(\"name\", \"Role044\"))).name"), is("Role044"));
		assertThat("incorrect selection", proxy.access("assignedRoles:select(1,eq(\"owner.name\", \"usr000\")).name"), is("Role060"));
		assertThat("incorrect selection", proxy.access("assignedRoles:select(1,eq(\"application\\\\.name\", \"FinInfo\")).name"), is("Role044"));
		...
		assertThat("incorrect selection", proxy.access("assignedRoles:select(eq(\"name\", \"Role044\"))"), is(List.of(bundle_Role044)));
		assertThat("incorrect selection", proxy.access("assignedRoles:select(like(\"displayName\", \" - \"))"), is(List.of(bundle_Role029, bundle_Role044)));
	}
	...
	public void test_switch()
	{
		...
		var proxy = OrionQL.wrap(identity, null);
		// Act & Assert
		assertThat("incorrect result", proxy.access("assignedRoles:map(:switch(name,60=owner.name,44=displayName,name))"), is(List.of("Role029", "Finance Information - Admin", "usr000")));
		assertThat("incorrect result", proxy.access("assignedRoles:map(name:switch(60=@.owner.name,44=@.displayName,@.name))"), is(List.of("Role029", "Finance Information - Admin", "usr000")));
